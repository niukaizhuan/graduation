...
function loginSuc(data) {
    ...
    if (data.obj.info === "success") {
        ...
        // 将菜单转成对象字符串
        let menuVoListStr = JSON.stringify(data.obj.menuVoList);
        // 将菜单放入会话存储中
        window.sessionStorage.setItem("menuVoList", menuVoListStr);
        // 将用户信息放入cookie中
        window.cookieUtil.set("userName", data.obj.userName);
        window.cookieUtil.set("password", data.obj.password);
        // 将token放入cookie中
        window.cookieUtil.set("Authorization", data.obj.token, '2h');
        // 跳转首页
