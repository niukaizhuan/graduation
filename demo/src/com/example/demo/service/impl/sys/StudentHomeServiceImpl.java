...
import com.example.demo.util.sys.MultipleAnswersUtil;
...
public class StudentHomeServiceImpl implements StudentHomeService {
	...
	public int insertPaperAnswerAndPaperScore(String sno, Integer paperId, List<String> singleAnswers,
		...
		for (int i = 0; i < fillQueList.size(); i++) {
			...
			if (fillAnswers.get(i) != null) {
				if (fillQueList.get(i).getFillAnswer().equalsIgnoreCase(fillAnswers.get(i).trim())) {
					...
					// fillQueList.get(i).getFillAnswer());
		...
		int updateResult = studentPaperScoreMapper.updateByExampleSelective(studentPaperScore,
				studentPaperScoreExample);
		return updateResult;
	}
	...
	public Map<String, Object> getScoreReport(String sno, Integer paperId) {
		...
		int totalScore = queNum.get("singleNum") * paper.getSingleScore()
				+ queNum.get("multipleNum") * paper.getMultipleScore() + queNum.get("judgeNum") * paper.getJudgeScore()
		...
		singleScoreMap.put("value",
				studentPaperAnswerMapper.selectSingleCorrectCount(studentPaperAnswerCorrect) * paper.getSingleScore());
		...
		multipleScoreMap.put("value", studentPaperAnswerMapper.selectMultipleCorrectCount(studentPaperAnswerCorrect)
				* paper.getMultipleScore());
		...
		judgeScoreMap.put("value",
				studentPaperAnswerMapper.selectJudgeCorrectCount(studentPaperAnswerCorrect) * paper.getJudgeScore());
		...
		fillScoreMap.put("value",
				studentPaperAnswerMapper.selectFillCorrectCount(studentPaperAnswerCorrect) * paper.getFillScore());
