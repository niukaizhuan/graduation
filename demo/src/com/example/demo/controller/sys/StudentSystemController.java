...
import com.example.demo.util.sys.EmailUtil;
import com.example.demo.util.sys.MobilePhoneUtil;
import com.example.demo.util.sys.ProfileImageSavaUtil;
...
public class StudentSystemController {
	...
	public ServerResponse studentRegister(@RequestBody Map<String, String> obj) {
		...
		if (isRegistered) {
			...
			int result = studentSystemService.registerStudent(sno, stuPsw, stuName, stuSex, stuEmail, stuPhone,
					new Date(), stuSecurityCode);
			if (result == 0)
	...
	public ServerResponse studentPswChange(@RequestBody Map<String, String> jsonObject, HttpServletRequest request) {
		 ...
		 * ServerResponse.createByError("会话失效，请重新登录"); }
		 */
	...
	public ServerResponse studentInfoChange(MultipartFile stuImgSrc, HttpServletRequest request) {
