...
public class StudentHomeController {
	...
	public ServerResponse getPapersInfoByPaperId(@RequestParam("paperId") Integer paperId) {
		...
		if (paper != null && numObj != null) {
			/*
			 * List listResult = new ArrayList(); listResult.add(paper);
	...
	public ServerResponse insertStudentPaperScore(@RequestBody(required = false) StudentPaperScore studentPaperScore) {
		...
		if (result == 0)
			...
			return ServerResponse.createBySuccess(
					"学号" + studentPaperScore.getSno() + "试卷id" + studentPaperScore.getPaperId() + "初始化插入学生成绩表信息成功",
					null);
	}
	...
	public ServerResponse submitPaper(@RequestBody Map<String, Object> obj) {
		...
		int result = studentHomeService.insertPaperAnswerAndPaperScore(sno, paperId, singleAnswers, multipleAnswers,
				judgeAnswers, fillAnswers, timeUsed);
		if (result == 0) {
	...
	public ServerResponse getScoreReport(@RequestParam("sno") String sno, @RequestParam("paperId") Integer paperId) {
