...
export default {
  ...
  async getRotationImages({commit}){
    ...
    if (result.obj.statu === 0) {
      const rotationImages = result.obj.data
  ...
  async getLanguagesInfo({commit}){
    ...
    // 提交一个mutation
    console.log("异步获取首页编程语言信息数组", result)
    if (result.obj.statu === 0) {
      const languagesInfo = result.obj.data
  ...
  async getUnreadMsgCount({commit, state}){
    ...
    if (result.obj.statu === 0) {
      const unreadMsgCount = result.obj.data;
  ...
  async receiveFeedbackInfo({commit, state}, setStore){
    ...
    if (result.obj.statu == 0){
      const feedbackInfo = result.obj.data;
  ...
  async getExamCalendar({commit}, setStore){
    ...
    // 提交一个mutation
    if (result.obj.statu === 0) {
      const examCalendar = result.obj.data
