    ...
    methods: {
      ...
      async insertStudentPaperScore() {
        ...
        if (result.obj.statu == 0) {
          sessionStorage.setItem("sno" + this.sno + "paperId" + this.paperId, result.obj.msg);
        }
        else {
          Toast({
      ...
      async getPaperInfoByPaperId(callback) {
        ...
        if (result.obj.statu == 0) {
          ...
          this.paperInfo = result.obj.data.paperInfo;
          this.queNumInfo = result.obj.data.queNumInfo;
          this.singleQueList = result.obj.data.singleQueList;
          this.multipleQueList = result.obj.data.multipleQueList;
          this.judgeQueList = result.obj.data.judgeQueList;
          this.fillQueList = result.obj.data.fillQueList;
          Indicator.close();
          callback && callback()
        }
        else {
      ...
      clickSubmit(){
        MessageBox.confirm('确定要提交试卷吗?').then(action => {
          ...
          console.log("用户手动点击提交试卷按钮", result)
          if (result){
            //等待成绩计算完毕并插入数据库表
            Indicator.open({
              text: '自动计算成绩中...',
      ...
      async handleSubmit() {
        ...
        console.log("将考试答案数据提交给后台", result)
        if (result.obj.statu == 0) {
          //交卷成功
          /*//清除sessionStorage数据
          sessionStorage.removeItem("currentIndex");
