    ...
    methods: {
      async getList() {
        ...
        if (result.obj.statu === 0){
          this.paperNameOptions = result.obj.data.papersList
          this.total = result.obj.data.scoresList.length
          this.list = result.obj.data.scoresList.filter((item, index) => index < this.listQuery.limit * this.listQuery.page && index >= this.listQuery.limit * (this.listQuery.page - 1))
        }
        // 延迟0.5秒等待请求数据
        setTimeout(() => {
      ...
      async handleDownloadAll(){
        this.downloadLoading = true
        let result = await reqGetScoresList()
        let list = result.obj.data.scoresList
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['学号', '姓名', '试卷名称', '考试分数', '开始考试时间', '提交考卷时间', '花费时间']
          const filterVal = ['sno', 'stuName', 'paperName', 'score', 'startTime', 'endTime', 'timeUsed']
