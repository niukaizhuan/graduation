    ...
    methods: {
      async getList() {
        ...
        if (result.obj.statu === 0){
          this.langOptions = result.obj.data.langOptions
          this.total = result.obj.data.papersList.length
          this.list = result.obj.data.papersList.filter((item, index) => index < this.listQuery.limit * this.listQuery.page && index >= this.listQuery.limit * (this.listQuery.page - 1))
        }
        // 延迟0.5秒等待请求数据
        setTimeout(() => {
      ...
      async seePaperDetail(row, column, event) {
        ...
        let singleData = result.obj.data.singleData
        let multipleData = result.obj.data.multipleData
        let judgeData = result.obj.data.judgeData
        let fillData = result.obj.data.fillData
        //初始化数据
        this.filterText = ''
      ...
      async handleFilter(){
        ...
        if (result.obj.statu === 0){
          this.total = result.obj.data.length
          this.list = result.obj.data.filter((item, index) => index < this.listQuery.limit * this.listQuery.page && index >= this.listQuery.limit * (this.listQuery.page - 1))
        }
        // 延迟一秒等待请求数据
        setTimeout(() => {
          this.listLoading = false
